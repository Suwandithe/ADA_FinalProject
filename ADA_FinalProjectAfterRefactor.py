...
@lru_cache(None)  # Cache the results of this function to avoid redundant API calls
...
def get_coordinates(place):
    ...
    try:
        ...
        # print(response_data)  # Debugging: Remove or comment out in production
        ...
        if response_data.get('status') == 'OK':
            ...
            raise ValueError(f"Error finding location for '{place}': {response_data.get('status')} - {error_message}")
...
def get_distance_matrix(locations, mode="driving"):
    ...
    if not locations or len(locations) < 2:
        raise ValueError("At least two locations are required to compute a distance matrix.")
    ...
    try:
        ...
        if response_data.get('status') != 'OK':
            error_message = response_data.get('error_message', 'Unknown error')
            raise ValueError(f"Distance Matrix API error: {response_data.get('status')} - {error_message}")

        # Parse the response into matrices
        distance_matrix = []
        duration_matrix = []
...
def dijkstra_greedy(dist_matrix):
    ...
    # Using a priority queue (heap) to efficiently find the nearest location
    pq = []
    for i in range(n):
        if i != current_location:
            heapq.heappush(pq, (dist_matrix[current_location][i], i))  # Push (distance, index)
    
    while pq:
        dist, nearest_location = heapq.heappop(pq)  # Get the nearest location
        if visited[nearest_location]:
            continue
        route.append(nearest_location)
        total_distance += dist
        visited[nearest_location] = True
        current_location = nearest_location
        # Refill the priority queue with updated distances
        for i in range(n):
            if not visited[i]:
                heapq.heappush(pq, (dist_matrix[current_location][i], i))
...
def tsp(dp_mask, pos):
    ...
    if dp_mask == (1 << num_cities) - 1:
        return DIST_MATRIX[pos][0]  # Cost to return to the starting point
    
    # Recursive case: explore unvisited cities
    min_distance = float('inf')
    for city in range(num_cities):
        # Check if city is unvisited
        if not (dp_mask & (1 << city)):
            new_mask = dp_mask | (1 << city)
            distance = DIST_MATRIX[pos][city] + tsp(new_mask, city)
